...
| **7** | Document Integrity | SHA-256 hash comparison between stored value and recomputed pre-encryption bytes. Backed by DocumentFileStore. | `backend/apps/documents/views.py`, `backend/apps/security/services.py` | `GET /api/documents/{id}/verify-integrity/` | `IntegrityVerification.tsx` | `test_integrity_verification.py` | **PASS** |
