...
| Feature | Current Implementation | Actual Status | Evidence | Problems | Required Action |
|---|---|---|---|---|---|
| **Document Integrity** | SHA-256 hash calculated pre-encryption and verified against database record. Supported by persistent DB-backed FileStore caching. | **FULLY HARDENED** | `backend/apps/documents/views.py`, `backend/apps/security/services.py` | Production Render containers have ephemeral storage. | DB-backed DocumentFileStore automatically backs up and restores encrypted evidence on-demand to survive restarts. |
| **Encrypted Storage** | Files encrypted using Fernet keys stored in environment variables and synced to DB. | **FULLY HARDENED** | `backend/apps/security/services.py`, `backend/apps/documents/models.py` | If `DOCUMENT_ENCRYPTION_KEY` changes, files become unreadable. | Note key persistence constraint in configuration guide. |
